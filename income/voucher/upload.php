...
$validate = true;
do{
    if(empty($_POST['incomeCode']) && $_POST['voucherNo']){
        $validate = false;
...
if($validate) {
    ...
    if($logo->uploaded){
        ...
        $path = ROOT."../Assistant_Users/".$_SESSION['s_id']."/income/".$_POST["incomeCode"]."/";
        //echo $path;
        $logo->Process($path);
        if($logo->processed){
            $path = "income/".$_POST["incomeCode"]."/".$logo->file_dst_name;
            $_POST["mode"] = "AI";
            $_POST["imagePath"] = $path;
            $_POST['voucherNo'] = intval($_POST['voucherNo']);
            $incomeCode = intval($_POST['incomeCode']);
            $settings = new \Assistant\Income\Voucher\IncomeVoucher($incomeCode);
